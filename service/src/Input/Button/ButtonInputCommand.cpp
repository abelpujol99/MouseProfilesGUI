...
ButtonInputCommand::ButtonInputCommand(std::unique_ptr<ICommand>&& commandOnPress, std::unique_ptr<ICommand>&& commandOnHold,
    std::unique_ptr<ICommand>&& commandOnRelease, Duration&& firstDelayInMillis, Duration&& millisToRepeatInput) :
        InputCommand(std::move(firstDelayInMillis), std::move(millisToRepeatInput)), _commandOnPress(std::move(commandOnPress)),
        _commandOnHold(std::move(commandOnHold)), _commandOnRelease(std::move(commandOnRelease))
{}
