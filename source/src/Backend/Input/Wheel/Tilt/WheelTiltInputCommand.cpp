...
WheelTiltInputCommand::WheelTiltInputCommand(std::unique_ptr<ICommand>&& commandOnPress, std::unique_ptr<ICommand>&& commandOnHold,
    ...
    Duration&& millisUntilInputStops) :
        InputCommand(std::move(firstDelayInMillis), std::move(millisToRepeatInput)),
        _millisUntilInputStops(std::move(millisUntilInputStops)), _commandOnPress(std::move(commandOnPress)),
